...
void rcvWaypointsCallBack(const nav_msgs::Path& wp) {
    ...
    for (int k = 0; k < (int)wp_list.size(); k++) {
        ...
        cout << "The " << k + 1 << "th waypoint is:" << endl << wp_list[k] << endl;
    }
...
void visWayPointTraj(MatrixXd polyCoeff, VectorXd time) {
    ...
    for (int seg = 0; seg < time.size(); seg++) {
        for (double t = 0.0; t < time(seg); t += 0.01, count += 1) {
            pos = getPosPoly(polyCoeff, seg, t);
            cur(0) = pt.x = pos(0);
            cur(1) = pt.y = pos(1);
            cur(2) = pt.z = pos(2);
...
Vector3d getPosPoly(MatrixXd polyCoeff, int seg, double t) {
    Vector3d ret;

    for (int dim = 0; dim < 3; dim++) {
        VectorXd coeff = polyCoeff.row(dim).segment(seg * _poly_num1D, _poly_num1D);
